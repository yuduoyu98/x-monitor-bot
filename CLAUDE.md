...
> **进度**:SP1-a/b ✅、SP2 全完成(a–e:状态/tick/持久化/调度循环 + TelegramSink + main 接线,37 测试)。bot 在新架构下代码可跑。待办:NitterSource(SP1-c)、旧代码清理(SP1-d)、admin_gui 重构(SP4)、sync_mode 过滤 & 回灌(后做)。
...
**每订阅可配**(subscriptions 表):`sync_mode`(media_only/all)、`poll_interval`、`fetch_limit`(探 gap 页大小,默认 20)、`skip_retweets`(转推+引用,默认开)。
...
| NitterSource | `src/source/nitter.py` | 待实现(SP1-c,降级源) |
| SyncEngine(状态+调度) | `src/sync_engine.py` ✅ | 状态逻辑 + run_tick + tick_account + run_once/run_loop(全完成) |
...
python -m src.verify_discovery <账号>... --backend scweet       # discovery 活体校验
SCWEET_AUTH_TOKEN=<token> pytest -k full_flow_live              # 全流程活体(Source→Engine→Sink→TG)
ruff format . && ruff check .
pytest -v
```

> 无 CLI/GUI(暂)。订阅是 state.db 里的状态;全流程测试/验证靠**入参**(账号传 `TEST_ACCOUNT`、token 走 env、TG 走 config.yaml),不依赖管理界面。GUI 重构 = SP4(可选)。
